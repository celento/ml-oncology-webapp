(function (global, factory) {
	...
	*/
	...
	function valueEqual$1(a, b) {
	  ...
	  if (aType === 'object') {
	    ...
	    if (aValue !== a || bValue !== b) return valueEqual$1(aValue, bValue);
	...
	function createBrowserHistory$1(props) {
	  ...
	  function checkDOMListeners(delta) {
	    ...
	    if (listenerCount === 1) {
	      ...
	      if (needsHashChangeListener) window.removeEventListener(HashChangeEvent$2, handleHashChange);
	    }
	  }
	...
	function createHashHistory$1(props) {
	  ...
	  function push(path, state) {
	    ...
	    transitionManager.confirmTransitionTo(location, action, getUserConfirmation, function (ok) {
	      ...
	      if (hashChanged) {
	        ...
	        var prevIndex = allPaths.lastIndexOf(createPath$1(history.location));
	  ...
	  function replace(path, state) {
	    ...
	    transitionManager.confirmTransitionTo(location, action, getUserConfirmation, function (ok) {
	      ...
	      var prevIndex = allPaths.indexOf(createPath$1(history.location));
	...
	*/
	...
	function (_React$Component) {
	  ...
	  _proto.handleClick = function handleClick(event, context$$1) {
	    ...
	    !isModifiedEvent(event) // ignore clicks with modifier keys
	        ...
	        var method = this.props.replace ? context$$1.history.replace : context$$1.history.push;
	    ...
	    return React.createElement(context.Consumer, null, function (context$$1) {
	      ...
	      return React.createElement("a", _extends$1({}, rest, {
	        onClick: function onClick(event) {
	          return _this.handleClick(event, context$$1);
	        },
	        href: href,
	        ref: innerRef
	      }));
...

	{
	  ...
	  var innerRefType = propTypes$1.oneOfType([propTypes$1.string, propTypes$1.func]);
	  Link.propTypes = {
	...
	function NavLink(_ref) {
	  var ariaCurrent = _ref["aria-current"],
	      ...
	      to = _ref.to,
	      rest = _objectWithoutPropertiesLoose$1(_ref, ["aria-current", "activeClassName", "activeStyle", "className", "exact", "isActive", "location", "strict", "style", "to"]);
	...
	{
	  var ariaCurrentType = propTypes$1.oneOf(["page", "step", "location", "date", "time", "true"]);
	  NavLink.propTypes = {
