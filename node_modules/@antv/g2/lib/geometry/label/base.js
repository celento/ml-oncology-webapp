...
var GeometryLabel = /** @class */ (function () {
    function GeometryLabel(geometry) {
        ...
        this.defaultLabelCfg = util_1.get(geometry.theme, 'labels', {}); // 默认样式
    }
    /**
    ...
    GeometryLabel.prototype.getLabelPoint = function (labelCfg, mappingData, index) {
        ...
        if (mappingData &&
            ...
            ['funnel', 'pyramid'].includes(util_1.isArray(mappingData.shape) ? mappingData.shape[0] : mappingData.shape)) {
            var maxX_1 = -Infinity;
        ...
        if (mappingData.shape === 'pyramid' && !mappingData.nextPoints && mappingData.points) {
            mappingData.points.forEach(function (p) {
                ...
                if ((util_1.isArray(p1.x) && mappingData.x.indexOf(p1.x) === -1) ||
                    (util_1.isNumber(p1.x) && mappingData.x !== p1.x)) {
    ...
    GeometryLabel.prototype.getLabelCfgs = function (mapppingArray) {
        ...
        util_1.each(mapppingArray, function (mappingData, index) {
            ...
            var labelCfg = tslib_1.__assign(tslib_1.__assign({ id: labelId, data: origin, // 存储原始数据
