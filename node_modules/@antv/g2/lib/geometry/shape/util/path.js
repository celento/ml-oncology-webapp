...
exports.smoothBezier = function (points, smooth, isLoop, constraint) {
    ...
    if (hasConstraint) {
        ...
        for (var i = 0, l = points.length; i < l; i++) {
            var point = points[i];
            min = matrix_util_1.vec2.min([], min, point);
            max = matrix_util_1.vec2.max([], max, point);
        }
        min = matrix_util_1.vec2.min([], min, constraint[0]);
        max = matrix_util_1.vec2.max([], max, constraint[1]);
    }
    for (var i = 0, len = points.length; i < len; i++) {
        ...
        var v1 = matrix_util_1.vec2.scale([], v, -d0);
        var v2 = matrix_util_1.vec2.scale([], v, d1);
        var cp0 = matrix_util_1.vec2.add([], point, v1);
        var cp1 = matrix_util_1.vec2.add([], point, v2);
        if (hasConstraint) {
            cp0 = matrix_util_1.vec2.max([], cp0, min);
            cp0 = matrix_util_1.vec2.min([], cp0, max);
            cp1 = matrix_util_1.vec2.max([], cp1, min);
            cp1 = matrix_util_1.vec2.min([], cp1, max);
        }
        cps.push(cp0);
        cps.push(cp1);
    }
