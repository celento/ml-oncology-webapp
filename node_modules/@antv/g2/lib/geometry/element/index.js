...
var Element = /** @class */ (function (_super) {
    ...
    Element.prototype.getStateStyle = function (stateName, shapeKey) {
        ...
        var stateOption = util_1.get(this.geometry.stateOption, stateName, {});
        var stateCfg = util_1.deepMix({}, util_1.get(theme, [shapeType, stateName], {}), stateOption);
        var shapeStyle = util_1.get(stateCfg.style, [shapeKey]) ?
            util_1.get(stateCfg.style, [shapeKey]) :
            stateCfg.style;
    ...
    Element.prototype.getAnimateCfg = function (animateType) {
        ...
        var defaultCfg = animate_1.getDefaultAnimateCfg(geometryType, coordinate, animateType);
        // 1. animate === false, 用户关闭动画
        // 2. 动画默认开启，用户没有对动画进行配置同时有没有内置的默认动画
        // 3. 用户关闭对应的动画  animate: { enter: false }
        if (!animate ||
            (animate === true && util_1.isEmpty(defaultCfg)) ||
            animate[animateType] === false ||
            animate[animateType] === null) {
            return null;
        }
        return tslib_1.__assign(tslib_1.__assign({}, defaultCfg), animate[animateType]);
    };
    // 绘制图形
    ...
    Element.prototype.getShapeDrawCfg = function (cfg) {
        return tslib_1.__assign(tslib_1.__assign({}, cfg), { defaultStyle: this.getStateStyle('default').style });
    };
    // 更新当前 shape 的样式
