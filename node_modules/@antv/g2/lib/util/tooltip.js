...
var getXDistance = util_1.memoize(function (scale) {
    ...
    var max = min;
    util_1.each(scaleValues, function (value) {
        // 时间类型需要 translate
...
function getTooltipValueScale(geometry) {
    ...
    var scale;
    util_1.each(attributes, function (attribute) {
...
function getTooltipName(originData, geometry) {
    ...
    if (groupScales.length) {
        // 如果存在分组类型，取第一个分组类型
        util_1.each(groupScales, function (scale) {
            nameScale = scale;
            return false;
        });
    }
...
function findDataByPoint(point, data, geometry) {
    ...
    if (util_1.isArray(firstXValue)) {
        util_1.each(data, function (record) {
        ...
        if (!xScale.isLinear && xScale.type !== 'timeCat') {
            ...
            util_1.each(data, function (record, index) {
                var originData = record[constant_1.FIELD_ORIGIN];
        ...
        if (last && next_1) {
            ...
            if (Math.abs(xScale.translate(last[constant_1.FIELD_ORIGIN][xField]) - xValue) >
                Math.abs(xScale.translate(next_1[constant_1.FIELD_ORIGIN][xField]) - xValue)) {
                last = next_1;
            }
        }
    }
