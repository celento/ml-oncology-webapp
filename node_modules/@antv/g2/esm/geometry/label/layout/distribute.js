...
function antiCollision(labelGroups, labels, lineHeight, plotRange, center, isRight) {
    ...
    labels.forEach(function (label) {
        var labelGroup = find(labelGroups, function (group) { return group.get('id') === label.id; });
        ...
        if (labelGroup) {
            ...
            if (labelLineShape) {
                ...
                // 贴近圆周
                var startPoint = polarToCartesian(center.x, center.y, angle, coordRadius);
                var inner = polarToCartesian(center.x, center.y, angle, coordRadius + distance / 2);
                var endPoint = {
 ...
 */
export function distribute(labels, shapes, items, region) {
    var offset = items[0] ? items[0].offset : 0;
    var lineHeight = items[0] ? get(items[0], 'labelHeight') : 14;
    ...
    if (coordinate && offset > 0) {
        ...
        halves_1.forEach(function (half, index) {
            ...
            if (half.length > maxLabelsCountForOneSide) {
                ...
                half.forEach(function (labelItem, idx) {
                    if (idx >= maxLabelsCountForOneSide) {
                        ...
                        var label = find(labels, function (label) { return label.get('id') === id_1; });
            ...
            var labelShapes = map(half, function (labelItem) {
                var id = labelItem.id;
                return find(labels, function (label) { return label.get('id') === id; });
            });
